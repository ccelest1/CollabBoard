...
const BOARDVERSE_ID = "BOARDVERSE";
...
function getBoardDisplayName(board: BoardRecord) {
  ...
  if (board.id.toUpperCase() === BOARDVERSE_ID) return "BOARDVERSE";
  return "BOARDVERSE";
}
...
function UniverseHubCard() {
  ...
  return (
    <section className="mb-8">
      <button type="button" onClick={() => router.push(`/board/${BOARDVERSE_ID}`)} className="group block w-full text-left">
        <div className="relative overflow-hidden rounded-lg border border-gray-200/50 bg-gradient-to-r from-gray-900 to-gray-700 px-6 py-12 text-center text-white shadow-2xl">
          ...
          <div className="relative z-10">
            <h2 className="text-4xl font-bold">BoardVerse Hub</h2>
...
export default function BoardverseClient({ currentUserId, userName }: Props) {
  const router = useRouter();
  ...
  useEffect(() => {
    const refreshBoards = () => {
      ...
      setBoards(all.filter((board) => board.id !== BOARDVERSE_ID));
    };
    refreshBoards();
    ...
    window.addEventListener("collabboard:boards-updated", refreshBoards);
    return () => {
      window.removeEventListener("storage", onStorageChange);
      window.removeEventListener("focus", refreshBoards);
      window.removeEventListener("collabboard:boards-updated", refreshBoards);
    };
  }, [currentUserId]);
            ...
            <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
              {allBoards.map((board) => (
                <Link
                  key={board.id}
                  href={`/board/${board.id}`}
                  className="block rounded-lg border border-slate-300 bg-white p-3.5 transition hover:shadow-md"
                >
